...
(function() {
  var SyncTubeClient, SyncTubeClient_ClipboardPoll, SyncTubeClient_CommandBar, SyncTubeClient_DropletUI, SyncTubeClient_History, SyncTubeClient_InputFilterUI, SyncTubeClient_KeyControls, SyncTubeClient_Player_HtmlFrame, SyncTubeClient_Player_HtmlImage, SyncTubeClient_Player_HtmlVideo, SyncTubeClient_Player_StuiBanned, SyncTubeClient_Player_StuiCreateForm, SyncTubeClient_Player_StuiKicked, SyncTubeClient_Player_Youtube, SyncTubeClient_PlaylistUI, ref;
  ...
  window.SyncTubeClient_ControlCodes = {
    ...
    CMD_taken_control: function() {
      var ref1, ref2;
      this.control = true;
      if ((ref1 = this.playlistUI) != null) {
        ref1.enableSorting();
      }
      return (ref2 = this.commandBar) != null ? ref2.show() : void 0;
    },
    CMD_lost_control: function() {
      ...
      return (ref2 = this.commandBar) != null ? ref2.hide() : void 0;
    },
        ...
        case "resume":
          if (data.cancelPauseEnsured) {
            if ((ref1 = this.player) != null) {
              if (typeof ref1.pauseEnsured === "function") {
                ref1.pauseEnsured("server cancel");
              }
            }
          }
          return (ref2 = this.player) != null ? ref2.play() : void 0;
        case "pause":
          return (ref3 = this.player) != null ? ref3.pause() : void 0;
        case "sync":
          return (ref4 = this.player) != null ? ref4.force_resync = true : void 0;
        case "seek":
          return (ref5 = this.player) != null ? ref5.seekTo(data.to, data.paused) : void 0;
        case "destroy":
          this.dontBroadcast = false;
          this.stopBroadcast();
          if (this.player) {
  ...
  window.SyncTubeClient_PlaylistUI.start = function() {
    return this.playlistUI = new SyncTubeClient_PlaylistUI(this, this.opts.playlist_ui);
  };
  ...
  window.SyncTubeClient = SyncTubeClient = (function() {
    class SyncTubeClient {
      ...
      constructor(opts = {}) {
        ...
        this.include(SyncTubeClient_InputFilterUI);
        this.include(SyncTubeClient_PlaylistUI);
        this.include(SyncTubeClient_DropletUI);
        ...
        if (this.getHashParams().keylock != null) {
          this.include(SyncTubeClient_KeyControls);
        }
        ref1 = this.included;
        for (j = 0, len = ref1.length; j < len; j++) {
          inc = ref1[j];
  ...
  window.SyncTubeClient_InputFilterUI = SyncTubeClient_InputFilterUI = class SyncTubeClient_InputFilterUI {
    constructor(client1, opts = {}) {
      ...
      if ((base1 = this.opts).filters == null) {
        base1.filters = [];
      }
      this.opts.secrets.push(/^\/system auth (.*)/);
      this.opts.filters.push(/^\/system auth (.*)/);
      this.opts.filters.push(/^\/control [^\s]+ (.*)/);
      this.opts.filters.push(/^\/password ([^\s]+).*/);
      this.hook();
    }

    getSecret(str) {
      var j, len, match, ref1, secret;
      ref1 = this.opts.secrets;
      for (j = 0, len = ref1.length; j < len; j++) {
        secret = ref1[j];
        if (match = str.match(secret)) {
          return [match, str, secret];
        }
      }
      return null;
    }
    ...
    getFilter(str) {
      var filter, j, len, match, ref1;
      ref1 = this.opts.filters;
      for (j = 0, len = ref1.length; j < len; j++) {
        filter = ref1[j];
        if (match = str.match(filter)) {
    ...
    filterCommand(str) {
      var fstr, i, j, len, m, ref1, x;
      m = this.getFilter(str);
      if (!m) {
        return str;
      }
      fstr = m[0][0];
    ...
    hook() {
      var oldAddSendCommand;
      this.client.input.on("keyup keydown change blur focus", (event) => {
        var m;
        if (m = this.getSecret(this.client.input.val())) {
          return this.client.input.attr("type", "password");
        } else {
          return this.client.input.attr("type", "input");
        }
      });
      oldAddSendCommand = this.client.addSendCommand;
      this.client.addSendCommand = (cmd, ...a) => {
        return oldAddSendCommand.call(this.client, this.filterCommand(cmd), ...a);
      };
      return this.client.debug("client input filtering enabled");
    }

  };

  window.SyncTubeClient_InputFilterUI.start = function() {
    return this.inputFilterUI = new SyncTubeClient_InputFilterUI(this, this.opts.input_filter_ui);
  };
  ...
  window.SyncTubeClient_KeyControls = SyncTubeClient_KeyControls = class SyncTubeClient_KeyControls {
    ...
    CMD_ui_keylock(data) {
      if (this.locked) {
        this.locked = false;
        return this.cage.blur();
      } else {
        return this.cage.focus();
      }
    }
    ...
    enableButton() {
      this.button.removeClass("btn-secondary").addClass("btn-warning");
      return this.button.find("i").removeClass("fa-unlock").addClass("fa-lock");
    }
  ...
  window.SyncTubeClient_KeyControls.start = function() {
    return this.KeyControls = new SyncTubeClient_KeyControls(this, this.opts.key_controls);
  };
  ...
  window.SyncTubeClient_DropletUI = SyncTubeClient_DropletUI = class SyncTubeClient_DropletUI {
    ...
    createDOM() {
      this.dropMask = $("<div id=\"ui_droplet\"></div>").appendTo(this.srow);
      return this.dropText = $("<h1 class=\"drophere\"></h1>").appendTo(this.dropMask);
    }
    ...
    createEventHandlers() {
      ...
      $(document).on("dragover", "#second_row", (ev) => {
        ...
        return this.dropMask.addClass('dragover');
      });
      $(document).on("dragleave dragend", "#second_row", (ev) => {
        return this.dropMask.removeClass('dragover');
      });
      ...
      $(document).on("drop", "#second_row", (ev) => {
        ...
        return ev.originalEvent.dataTransfer.items[0].getAsString((str) => {
          return this.client.sendCommand("play", str);
        });
      });
      return $(document).on("drop", (ev) => {
        $("body").removeClass('dragover');
        this.dropMask.removeClass('dragover');
        ev.preventDefault();
  ...
  window.SyncTubeClient_DropletUI.start = function() {
    return this.DropletUI = new SyncTubeClient_DropletUI(this, this.opts.droplet_ui);
  };
  ...
  window.SyncTubeClient_UI = {
    ...
    addMessage: function(data) {
      ...
      this.content.append(`<p>\n  <${tagname} style="color:${data.author_color}">${data.author}</${tagname}>\n  @ ${`0${dt.getHours()}`.slice(-2)}:${`0${dt.getMinutes()}`.slice(-2)}\n  <span style="color: ${data.text_color}">${(data.escape ? this.escapeHtml(data.text) : data.text)}</span>\n</p>`);
      return this.content.scrollTop(this.content.prop("scrollHeight"));
    },
