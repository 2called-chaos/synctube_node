...
(function () {
  ...
  window.SyncTubeClient_ClipboardPoll = SyncTubeClient_ClipboardPoll =
  /*#__PURE__*/
  function () {
    function SyncTubeClient_ClipboardPoll(client1) {
      var opts = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

      _classCallCheck(this, SyncTubeClient_ClipboardPoll);
    ...
    _createClass(SyncTubeClient_ClipboardPoll, [{
      key: "detectAutostart",
      value: function detectAutostart() {
        var _this21 = this;

        return navigator.permissions.query({
          name: 'clipboard-read'
        }).then(function (status) {
          if (_this21.opts.autostartIfGranted && status.state === "granted") {
            _this21.start();
          }

          return status.onchange = function () {
            if (status.state === "granted") {
              return _this21.start();
            } else {
              return _this21.stop();
            }
          };
          ...
          return regeneratorRuntime.wrap(function _callee$(_context) {
            while (1) {
              switch (_context.prev = _context.next) {
                case 0:
                  wait = _args.length > 0 && _args[0] !== undefined ? _args[0] : true;
                  this.run = false;
                  this.client.debug("Stopping clipboard polling...");
                  promise = new Promise(function (resolve, reject) {
                    return _this22.stopped = resolve;
                  });
            ...
            while (1) {
              switch (_context2.prev = _context2.next) {
                ...
                case 8:
                  ...
                  this.process(text);
                  return _context2.abrupt("return", this.lastValue = text);
      ...
      key: "process",
      value: function process(val) {
        return this.client.debug("Processing", val);
      }
    }]);

    return SyncTubeClient_ClipboardPoll;
  }();
