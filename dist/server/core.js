...
(function() {
  // libraries
  var COLORS, Channel, Client, Commands, HttpRequest, PersistedStorage, PlaylistManager, SyncTubeServer, UTIL, fs, http, webSocketServer;
  ...
  exports.Class = SyncTubeServer = class SyncTubeServer {
    constructor(opts = {}) {
      ...
      this.banned = new PersistedStorage(this, "server/banned_ips");
      this.clients = [];
      this.channels = {};
    ...
    registerShutdownHandler() {
      process.on('exit', () => {
        return this.persistChannels();
      });
      process.on('SIGINT', () => {
        this.warn("Interrupt signal received, shutting down...");
        return process.exit(130);
      });
      process.on('SIGTERM', () => {
        this.warn("Termination signal received, shutting down...");
        return process.exit(2);
      });
      return process.on('uncaughtException', (e) => {
        this.warn("Encountered FATAL exception, shutting down...");
        this.error(`Uncaught FATAL Exception: ${e}`);
        console.trace(e);
    ...
    persistChannels() {
      var c, err, n, ref, results;
      this.info("Saving channels...");
      ref = this.channels;
      results = [];
      for (n in ref) {
        c = ref[n];
        try {
          this.debug(`Saving channel ${n}...`);
          results.push(c.persisted.sSave());
        } catch (error) {
          err = error;
          results.push(this.error(`Failed to save channel ${n}: ${err}`));
        }
      }
      return results;
    }
    ...
    loadPersistedChannels() {
      var cpath, err, file, files, i, jdata, len, results;
      cpath = `${this.root}/data/channel`;
      if (!fs.existsSync(cpath)) {
        return;
      }
      files = fs.readdirSync(cpath);
      this.info(`Restoring ${files.length} persisted channels...`);
      results = [];
      for (i = 0, len = files.length; i < len; i++) {
        ...
        try {
          ...
          if (!jdata.name) {
            throw "no name in JSON";
          }
          if (this.channels[jdata.name]) {
            throw `channel ${jdata.name} already exists`;
          }
          results.push(this.channels[jdata.name] = new Channel(this, jdata.name, jdata.password));
        } catch (error) {
          err = error;
          this.error(`Failed to restore channel from file ${file}: ${err}`);
          results.push(console.trace(err));
        }
      }
      return results;
    }
    ...
    guardBanned(client, reason) {
      ...
      if (!this.banned.hasKey(client.ip)) {
        return false;
      }
      match = this.banned.get(client.ip);
      // check expired
      if (match && (new Date) > match) {
        this.debug(`Purge expired ban for ${client.ip} which expired ${match}`);
        this.banned.purge(client.ip);
        return false;
      }
